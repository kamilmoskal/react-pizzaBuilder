...
const Auth = props => {
  ...
  if (props.ingSelected && props.isAuth){
    ...
    <Formik
      ...
      render={({
        ...
        <Form onSubmit={handleSubmit} className={classes.Auth}>
          ...
          {props.error ? <p className={classes.ErrorMsg}>authenticate error: {props.error.message.replace(/_/g, " ")}</p> : null}
          <Field 
          ...
          <div style={{display: 'flex'}}>
            <Button btnType='Primary' clicked={signInHandle} disabled={isValidating}>Sign In</Button>
            <Button btnType='Primary' clicked={signUpHandle}>Sign Up</Button>
          </div>
        </Form>
      )}
    />
